...
import { CardPotrait } from '@/components';

describe('CardPotrait render correctly', () => {
  const data = {
    sub_category: {
      category_url: '',
      name: 'Fashion',
  ...
  it('Binding data correctly', () => {
    render(<CardPotrait data={data} heightimg={436} />);
  ...
  it('Height container img', () => {
    render(<CardPotrait data={data} heightimg={436} />);
    expect(screen.getByTestId('card-image-container')).toHaveStyle(
      'height: 436px'
    );
  });
});
