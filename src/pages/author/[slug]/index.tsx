...
export default function PageAuthor({ isDesktop, isMobile }: DeviceViewProps) {
    ...
    <Main
      ...
      meta={
        <Meta
          ...
          url={`https://www.popbela.com/author/${data?.user?.username}`}
        />
      }
      isDesktop={isDesktop}
      isMobile={isMobile}
...
export const getServerSideProps: GetServerSideProps = async ({
  ...
  if (isPageAvailable) {
    const { redirection }: IRedirection = await getRedirectionPages(
      `v1/redirection?url=/author/${slug}`
    );
